...
class Compose(object):
    ...
    def __call__(self, image, label, boundary):
        for t in self.segtransform:
            image, label, boundary = t(image, label, boundary)
...
class RandRotate(object):
    ...
    def __call__(self, image, label, boundary):
        if random.random() < self.p:
            ...
            # matrix0 = cv2.getRotationMatrix2D((w / 4, h / 4), angle, 1)
            boundary = cv2.resize(boundary, (w, h), cv2.INTER_NEAREST)
            ...
            boundary = cv2.warpAffine(boundary, matrix, (w, h), flags=cv2.INTER_NEAREST,
                                      borderMode=cv2.BORDER_CONSTANT,
                                      borderValue=self.ignore_label)
            boundary = cv2.resize(boundary, (int(w / 2), int(h / 2)), cv2.INTER_NEAREST)
        return image, label, boundary
